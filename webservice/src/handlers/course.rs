...
mod tests {
    ...
    #[actix_rt::test] // 使用 actix_rt 宏来异步运行测试
    ...
    async fn update_course_success() {
        ...
        let update_course = UpdateCourse {
            ...
            format: Some("Updated format".into()),
            structure: Some("Updated structure".into()),
            duration: Some("Updated duration".into()),
            price: Some(100),
            language: Some("Updated language".into()),
            level: Some("Updated level".into()),
        };
        let path_params: web::Path<(i32, i32)> = web::Path::from((1, 2));
        let update_body = web::Json(update_course);
        let resp = update_course_details(app_state, path_params, update_body).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
    ...
    async fn delete_course_failure() {
        ...
        match resp {
            Ok(_) => println!("Expected error, got OK"),
            Err(err) => assert_eq!(err.status_code(), StatusCode::NOT_FOUND),
        }
    }
}